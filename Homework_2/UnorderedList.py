...
class UnorderedList:
    ...
    # Problem 5 https://piazza.com/class/jtqbsk4spmk58r?cid=147
    def removeMin(self):
        #raise NotImplementedError
        '''current = self.head
        lists = []
        min_num = current.getData()
        lists.append(min_num)
        ...
        current, next = self.head, self.head.getNext()
        min_num = current.getData() # search minimum number search(item)

        while next is not None: #keep searchinggg
            current, next = next, next.getNext()
            if current.getData() < min_num: #if 
                min_num = current.getData() # saving min_num through getData
                

        self.remove(min_num) #remove(item)
