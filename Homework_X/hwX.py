...
def merge(left, right):
    if left == []:
        ...
        return left
    elif left[0][1] <= right[0][1]: ####fix this line!!! SLAC stable merge sort
...
def selectionSort(alist):
    for i in range(0,len(alist) - 1):
        ...
        for j in range(i + 1, len(alist)):
            if alist[j][1] < alist[min][1]: ##unstable selection sort
                min = j
        temp = alist[i]
        alist[i] = alist[min]
        alist[min] = temp
...
problem3_part2 = [89,63,35 ,29,24,19,17,1,7,3,4]
problem3_part3 = [35,29,24,19 ,17,1,7,3,4]
